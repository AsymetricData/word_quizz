...
// build/dev/javascript/word_quizz/model.mjs
var ApplicationStarted = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserSubmitResponse = class extends CustomType {
...
var UserClickedNext = class extends CustomType {
};
var Model2 = class extends CustomType {
  constructor(step, state, current_question, questions) {
    super();
    this.step = step;
    this.state = state;
...
var UserShouldAnswer = class extends CustomType {
};
var UserCanSeeSolution = class extends CustomType {
};
var NoStep = class extends CustomType {
};
var QuizzState = class extends CustomType {
  constructor(score, history) {
    super();
    this.score = score;
...
var Question = class extends CustomType {
  constructor(word, choices, answer, solution2) {
    super();
    this.word = word;
...
var QuestionSet = class extends CustomType {
...
function new_model() {
  return new Model2(
    new NoStep(),
    new QuizzState(0, toList([])),
    new None(),
    toList([])
  );
}

// build/dev/javascript/word_quizz/question.mjs
function question_decoder(dynamic) {
  return decode1(
    (var0) => {
      return new QuestionSet(var0);
    },
    field(
      "questions",
      list(
        decode4(
          (var0, var1, var2, var3) => {
            return new Question(var0, var1, var2, var3);
          },
          field("word", string),
...
// build/dev/javascript/word_quizz/ui.mjs
function style_nav_bar() {
  return toList([
...
function style_game_screen_container() {
  return toList([
    ["margin", "2rem auto"],
    ["max-width", "800px"],
    ["font-family", "Courier, monospace"],
...
function style_solution_container() {
  return toList([
    ["margin-top", "1rem"],
    ["padding", "1rem"],
...
function nav_bar(model) {
  return nav(
    toList([style(style_nav_bar())]),
    toList([
      text2(
        "Score : " + to_string(model.state.score) + "/" + to_string(
          length2(model.state.history)
        )
      )
    ])
  );
...
function solution(m) {
  ...
  let $1 = m.step;
  if ($1 instanceof UserCanSeeSolution) {
    return div(
      toList([]),
      toList([
        div(
          toList([style(style_solution_container())]),
          toList([text2(current_question.solution)])
        ),
        div(
          toList([style(style_separation())]),
          toList([
            button(
              toList([
                style(style_next_button()),
                on_click(new UserClickedNext())
              ]),
              toList([text2("Suivant")])
            )
          ])
        )
      ])
...
function game_screen(model, on_click2) {
  return guard(
    (() => {
      let _pipe = model.current_question;
      return is_none(_pipe);
    })(),
    text2("Unable to load the current question. Please try again..."),
    () => {
      let $ = model.current_question;
      if (!($ instanceof Some)) {
        throw makeError(
          "let_assert",
          "ui",
      ...
      let current_question = $[0];
      return div(
        toList([style(style_game_screen_container())]),
        toList([
          h1(
            toList([style(style_question_title())]),
            toList([
              text2(
                "Quel est le sens du mot `" + current_question.word + "`"
              )
            ])
          ),
          div(
            ...
            (() => {
              let _pipe = current_question.choices;
              return map2(
                _pipe,
                (choice) => {
                  return button(
                    toList([
                      style(style_choice_button()),
                      (() => {
                        let $1 = model.step;
                        if ($1 instanceof UserCanSeeSolution) {
                          return none2();
                          ...
                          return on_click(
                            on_click2(current_question, choice)
                          );
                        }
                      })()
                    ]),
...
// build/dev/javascript/word_quizz/word_quizz.mjs
function view(model) {
  return div(
    toList([]),
    toList([
      nav_bar(model),
      game_screen(model, (a, b) => {
        return new UserSubmitResponse(a, b);
      })
    ])
  );
}
function init_app() {
  return get(
    ...
    expect_json(
      question_decoder,
      (var0) => {
        return new ApplicationStarted(var0);
      }
    )
  );
}
function init2(_) {
  return [new_model(), init_app()];
}
function init_question(model, questions) {
  let $ = (() => {
    let _pipe = questions.questions;
  ...
  let current_question = $[0];
  let new_model2 = new Model2(
    new UserShouldAnswer(),
    new QuizzState(0, toList([])),
    new Some(current_question),
    questions.questions
  );
  return [new_model2, none()];
}
function handle_submit_answer(question, answer, m) {
  ...
  let model = new Model2(
    new UserCanSeeSolution(),
    new QuizzState(score, prepend(question, m.state.history)),
    m.current_question,
    m.questions
  );
  return [model, none()];
}
function handle_next_question(m) {
  let $ = (() => {
    let _pipe = m.questions;
    let _pipe$1 = shuffle(_pipe);
    return last(_pipe$1);
  })();
  if (!$.isOk()) {
  ...
  let next = $[0];
  let next$1 = next.withFields({
    choices: (() => {
      let _pipe = next.choices;
      return shuffle(_pipe);
    })()
  });
  return [
    m.withFields({
      current_question: new Some(next$1),
      step: new UserShouldAnswer()
    }),
    none()
  ];
}
function update(model, msg) {
  if (msg instanceof ApplicationStarted && msg[0].isOk()) {
    let question_set = msg[0][0];
    return init_question(model, question_set);
  } else if (msg instanceof ApplicationStarted && !msg[0].isOk()) {
    let err = msg[0][0];
    debug(err);
    return [model, none()];
  } else if (msg instanceof UserSubmitResponse) {
    let question = msg.question;
    let answer = msg.response;
    return handle_submit_answer(question, answer, model);
  } else if (msg instanceof UserClickedNext) {
